 ...
 * @author gadv1
 */
public class Lacteo extends Ingredientes{
    ...
    public Lacteo(int dia, int mes, int anio, double cantidad, String nombre, String unidad) {
        super(dia, mes, anio, cantidad, nombre, unidad);
    }
    
    public void llenarTablaLacteo(LinkedList<Lacteo> l, JTable tabla,LacteoXML lcXML){
        if(!l.isEmpty()){
          ...
          Object[] row;
            for(int i = 0; i < l.size() ; i++){
                if(l.get(i).verificarVencido(l.get(i).getDia(), diaA, l.get(i).getMes(), mesA, l.get(i).getAnio(), anioA)){
                   ...
                   lcXML.actualizarLacteo(l.get(i));
                   row = new Object[5];
    ...
    public Lacteo generarFecha(Lacteo l){
